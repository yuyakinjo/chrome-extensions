...
```sh
bun install
bun run build     # dist/ に拡張機能一式が出ます
```
...
読み込むのはリポジトリのルートではなく `dist/` です。ルートには `.ts` しか無いので Chrome は読めません。

Chrome 111 以上が必要です（`chrome.tabGroups` / `chrome.storage.session` / `scripting` の `world: "MAIN"`）。

**コードを更新したら `bun run build`（または `bun run dev`）でビルドし直してから、
...
tsconfig.json          拡張機能本体（src/ → dist/src/）
...
tools/
  build.ts             dist/ の組み立て（tsc + 静的ファイルのコピー）
  make-icons.ts        アイコン生成（依存なし・`bun run icons`）
icons/                 生成済み PNG
...
bun run build       # dist/ を作り直す
bun run dev         # tsc --watch + html/css/icons の監視
bun run typecheck   # 出力せずに型だけ見る（src と tools の両方）
bun run clean       # dist/ を消す
