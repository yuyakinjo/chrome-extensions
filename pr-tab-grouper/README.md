...
- **自分のオープン PR を全部開く** — GitHub 上の自分の open PR を取得して、未オープンのものを開いてグループ化（非表示にした PR は除く）
...
- **対応済みの PR はタブを閉じるだけで隠せる** — レビューを終えてマージを待つだけの PR は、タブを閉じれば開き直さない。要修正・CI 失敗・コンフリクトが来たときだけ戻ってくる
...
| 取得のたびに、開いていない自分の PR をタブで開く | ON | 取得間隔ごとに「1 PR = 1 タブ」に揃える。足りないタブだけを裏で開いてグループに入れる。手で閉じた PR は非表示として覚えるので開き直さない |
...
レビューを終えてマージを待つだけの PR は、**タブを閉じるだけで非表示になります**。閉じたタブは
次の取得でも開き直しません。タブグループごと閉じれば、その中の PR がまとめて非表示になります。

ポップアップの PR 一覧からも切り替えられます。行の右の **「非表示」** でタブを閉じて非表示に、
**「戻す」** でタブを開き直します。非表示にした PR は一覧の下にまとめて薄く表示され、見出しに
`(5 / 非表示 2)` のように件数が出ます。

自分で戻さなくても、次の 2 つでは自動的に解除されます。

- **要対応に転じたとき** — 修正がリクエストされた / CI が失敗した / コンフリクトした。つまり
  「対応済み」でなくなった瞬間にタブが戻ってきます。判定は通知とまったく同じものを使っているので、
  通知が飛んだ PR は必ずタブにも現れます
- **マージ・クローズされたとき** — 印ごと捨てます

承認されただけ（`APPROVED`）では戻りません。マージ待ちのまま置いておけます。

> **タブが 1 枚だけのウィンドウでは、閉じても非表示になりません。** Chrome はタブを閉じた操作と
> ウィンドウを閉じた操作を区別できず（`isWindowClosing`）、ウィンドウごと閉じたときに全 PR を
> 非表示にしてしまうのを避けるためです。この場合はポップアップの「非表示」を使ってください。
...
- **非表示にした PR は開きません** — 手で閉じたタブとポップアップの「非表示」を印として覚えていて、
  そこに載っている PR は対象から外します。印は `chrome.storage.local` に置くので、ブラウザを
  閉じても残ります（マージ待ちは数日かかるため）。閉じられたタブがどの PR だったかは
  `chrome.tabs.onRemoved` では読めないので、タブごとの PR を `chrome.storage.session` に控えています
...
src/
  ...
  lib/
    ...
    hidden.ts          「開き直さない PR」の印（非表示）と、その自動解除
...
- タブが 1 枚だけのウィンドウでそのタブを閉じても非表示になりません（ウィンドウを閉じた操作と
  区別できないため）。ポップアップの「非表示」を使ってください
